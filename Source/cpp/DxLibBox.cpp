...
void DxLibBox::Create(float width, float height, float depth)//幅、高さ、奥行
{
	width  /= 2;
	height /= 2;
	depth  /= 2;
...
{
	graph = Master::resManager->LoadImageFromFile(texName);
}
...
void DxLibBox::Transform(void)//ワールド変換
{
...
void DxLibBox::Draw(void)//描画
{
	// ワールドマトリクスをテンポラリーの頂点へ反映する
	...
	{
		DrawPolygonIndexed3D(tempVertex, 24, index, 12, DX_NONE_GRAPH, TRUE);
	}
}
